...
-- ============================================================
-- 038_cancelamento_lancamento_semi_acabado_HML_VALIDACAO_GAIA.sql
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'homologacao' AND table_name = 'semi_acabado_lancamento' AND column_name = 'cancelado_em') THEN
        RAISE EXCEPTION 'VALIDACAO 038: coluna cancelado_em ausente.';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'homologacao' AND table_name = 'semi_acabado_lancamento' AND column_name = 'cancelado_por') THEN
        RAISE EXCEPTION 'VALIDACAO 038: coluna cancelado_por ausente.';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'homologacao' AND table_name = 'semi_acabado_lancamento' AND column_name = 'motivo_cancelamento') THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM homologacao.semi_acabado_lancamento WHERE codigo_semi_acabado_lancamento = v_lancamento AND status_lancamento = 'CANCELADO') THEN
...
\echo 'OK - regras de cancelamento local do Produto Semiacabado validadas (HML)'
