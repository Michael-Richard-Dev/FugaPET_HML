...
-- ============================================================
-- 052_excluir_pesagem_auditoria_permissao_Q_DRYRUN_GAIA.sql
-- Projeto FugaPET | Ambiente Q | Schema homologacao
-- Incremento 052 - Excluir Pesagem
-- DRY-RUN ESTRUTURAL / ROLLBACK / SEM DML DE PERMISSAO
--
-- OBJETIVO:
...
-- ============================================================
-- 1. PERMISSAO - DRY-RUN SEM DML/SEQUENCE
-- ============================================================
...
BEGIN
    ...
    IF v_count <> 0 THEN
        RAISE EXCEPTION
            '052 DRYRUN ABORTADO: baseline permission count = %, esperado 0.',
            v_count;
    END IF;
END $$;
...
BEGIN
    ...
    IF v_permission_count <> 0 THEN
        RAISE EXCEPTION
            '052 DRYRUN ABORTADO: permission count alterado durante dry-run = %, esperado 0.',
            v_permission_count;
    END IF;
...
BEGIN
    ...
    IF v_outbox <> 0 OR v_tentativa <> 0 THEN
        RAISE EXCEPTION
            '052 DRYRUN ABORTADO: SAP guard retornou outbox=% tentativa=%.',
            v_outbox,
            v_tentativa;
    END IF;
END $$;
...
\echo '============================================================'
\echo '052_DRYRUN_ROLLBACK_EXECUTED'
\echo '============================================================'
\echo 'PERMISSION_DML_EXECUTED=NAO'
\echo 'AUDIT_DML_TEST_EXECUTED=NAO'
\echo 'USER_SEQUENCE_CONSUMPTION_INTENTIONAL=NAO'
\echo 'PERSISTENT_DB_CHANGE_EXPECTED=NAO'
\echo 'SAP_WRITE=0'
