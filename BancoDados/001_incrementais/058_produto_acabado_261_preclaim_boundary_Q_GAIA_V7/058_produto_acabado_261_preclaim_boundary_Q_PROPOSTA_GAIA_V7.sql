...
BEGIN
    ...
    -- Baseline 057: trigger historico 045 ainda deve apontar para o
    -- append-only original. O prepared-claim guard 058 ainda NAO pode existir.
    IF NOT EXISTS (
        SELECT 1
          FROM pg_trigger t
        ...
        RAISE EXCEPTION
          '058 PRECHECK: baseline trigger tentativa nao aponta para fn_pa_045_historico_append_only';
    END IF;

    IF to_regprocedure(
        'homologacao.fn_pa_045_etapa_tentativa_prepared_claim_guard()'
    ) IS NOT NULL THEN
        RAISE EXCEPTION
